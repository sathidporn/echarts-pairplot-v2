...
export default function ExploreDataPage() {
    ...
    let updateSamplingData = {}
      ...
      sensors.map((sensor) => {
        ...
        for (let i = 0; i < timestampsIndex.length; i++) {
            ...
            result = generateSamplingData({values: valuesArr, type})
            values.push(result)
        }
        if(values){
            let newObj = {}
  ...
  const onAddSpecialSensor = useCallback((specialSensor) => {
    if(specialSensor){
      ...
      let specialSensorData = generateSpecialSensor({data: filteredSensors, specialSensor})
    ...
    if(list !== undefined){
        let sensorList = list.filter(sensor=>sensor.SENSOR_TAG !== "").map((sensor) => {
          let index = sensors.findIndex(obj => obj.tag === sensor.SENSOR_TAG)
            return {
                status: index !== -1 ? "available" : "unavailable", 
                tag: index !== -1 ? sensors[index].tag : sensor.SENSOR_TAG, 
                checked: index !== -1 ? sensors[index].checked : false, 
  ...
  const onReadSpecialSensorListFile = useCallback((list) => {
    if(list !== undefined){
      ...
      // Generate existing special sensor data when import special sensor file
      if(sensors.filter(sensor=>sensor.status === "unavailable").length > 0){
        let updateFilteredSensors = filteredSensors
        let updateSensors = []
        // Find special sensor tag to generate data
        sensors.filter(sensor=>sensor.status === "unavailable").map((sensor) => {
          // Get duplicate special sensor tag rows
          let filterSensor = specialList.filter(s=>s.specialTag === sensor.tag)
          let derivedSensors = []
          filterSensor.map((sensor) => {
            derivedSensors.push({tag: sensor.derivedFromTag})
            return derivedSensors
          })
          // Define special sensor object
          let specialSensorObj = {}
          // Generate special sensor data
          let specialSensorData = generateSpecialSensor({data: updateFilteredSensors, specialSensor: {sensor: derivedSensors, tag: sensor.tag, name: sensor.name, calType: filterSensor[0].calType, subType: filterSensor[0].subType, constant: filterSensor[0].factor}})
          specialSensorObj[`${sensor.tag}`] = specialSensorData
          ...
          if (index !== -1 &&  updateFilteredSensors[sensor.tag].length > 0) {
            if(updateSensors.length === 0){
              updateSensors = [...sensors.slice(0, index), { ...sensors[index], status: "available"}, ...sensors.slice(index + 1)]
            }else{
        ...
        setOpen(true)
        setMessage("Cannot generate special sensor data. Please upload sensor file before upload special sensor file.") 
      }
  ...
  },[sensors, filteredSensors, setSensors, setSpecialSensors, setFilteredSensors])
                ...
                <Step key={step.label} className={classes.stepLabel}>
                  ...
                  <StepContent TransitionProps={{ unmountOnExit: false }}>
                    ...
                    <Grid item container xs={12} sm={12} md={12} lg={12} direction="row" justifyContent="space-between" spacing={1}>
                      ...
                      <Grid item xs={12} sm={12} md={12} lg={12}>
                        <AddSpecialSensor sensors={sensors.filter(sensor=>sensor.status !== "unavailable")} specialSensors={specialSensors} onAddSpecialSensor={onAddSpecialSensor} onCustomizeSensors={onUpdateSpecialSensors} onRemoveSpecialSensor={onRemoveSpecialSensor} onReadSpecialSensorListFile={onReadSpecialSensorListFile}></AddSpecialSensor>
                      </Grid>
                    </Grid>
                    </>
                    }
